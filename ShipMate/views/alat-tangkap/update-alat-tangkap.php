...
include '../../controller/AlatTangkapController.php';
$AlatTangkapController = new AlatTangkapController;

$id = $_GET["id"];
$alat_tangkap = $AlatTangkapController->find($id);

$success = false;
$failed = false;
if(isset($_POST["submit"])) {
    $result = $AlatTangkapController->update($_POST, $id);
...
<head>
    ...
    <title>Create Alat Tangkap</title>
...
<nav class="bg-teal-600 border-gray-200 px-2 h-20 flex items-center fixed w-full z-50">
  ...
  <?php if($success) : ?>
        ...
        <div class="bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-6 space-y-6">
                <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                    Alat tangkap sudah berhasil diupdate
                </p>
            </div>
            <!-- Modal footer -->
    ...
    <div class="relative w-1/3">
        ...
        <div class="bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-6 space-y-6">
                <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                    Terjadi kesalahan pada proses update alat tangkap
                </p>
  ...
  <img src="../../images/illustrations-14.svg" alt="" class="absolute -z-10 w-2/5 right-20 top-20 rotate-12">
    <div class="w-1/3 flex flex-col gap-10 items-center">
        <p class="text-4xl font-bold text-gray-500">Update Alat Tangkap</p>
        <form action="" method="post" class="w-full flex flex-col items-center gap-6 shadow-lg p-6 bg-gray-50 rounded border-2">
        <div class="relative w-full">
            <input type="text" id="nama_alat_tangkap" class="block rounded-t-lg px-2.5 pb-2.5 pt-5 w-full text-sm text-gray-900 border-0 border-b-2 border-gray-300 appearance-none dark:border-gray-600 dark:focus:border-teal-500 focus:outline-none focus:ring-0 focus:border-teal-600 peer" placeholder=" " name="nama_alat_tangkap" autocomplete="off" value="<?= $alat_tangkap["nama_alat_tangkap"] ?>"/>
            <label for="nama_alat_tangkap" class="absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-4 scale-75 top-4 z-10 origin-[0] left-2.5 peer-focus:text-teal-600 peer-focus:dark:text-teal-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-4">Nama Alat Tangkap</label>
        </div>
        <div class="w-full relative">
            <button type="submit" name="submit" class="w-full bg-teal-700 border-teal-700 border-4 active:border-4 focus:border-teal-600 active:border-teal-600 py-2 text-white  font-semibold rounded-lg uppercase">
                update
            </button>
        </div>
        </form>
